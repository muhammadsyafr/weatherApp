...
export const fetchData = async (loadContent) => {
  if(loadContent === 'forecast'){
    try {
      const res = await axios.get(
        `${weatherService.url}/forecast?q=${weatherService.city}&appid=${weatherService.key}&units=metric`
      );
      return res.data;
    } catch (error) {}
  } else {
    try {
      const {
        ...
        `${weatherService.url}/weather?q=${weatherService.city}&appid=${weatherService.key}&units=metric`
      );
      const modifiedData = {
        main,
        name,
